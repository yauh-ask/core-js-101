...
function chainPromises(array, action) {
  const promiseWithArray = new Promise((resolve) => {
    resolve(array);
  });
  return promiseWithArray
    .then((arrayWithValues) => {
      const arrayWithPromises = [];
      arrayWithValues.forEach((el) => {
        el.then((element) => arrayWithPromises.push(element));
      });
      return arrayWithPromises;
    })
    .then((arrayWithFinalData) => arrayWithFinalData.reduce(action));
}
