 ...
 *   getPolynom()      => null
 */
function getPolynom(...argumentsOfFunction) {
  const numbersOfArguments = argumentsOfFunction.length;
  if (numbersOfArguments === 3) {
    return (x) => x * (argumentsOfFunction[0] * x)
      + x * argumentsOfFunction[1]
      + argumentsOfFunction[2];
  }
  if (numbersOfArguments === 2) {
    return (x) => x * argumentsOfFunction[0] + argumentsOfFunction[1];
  }
  if (numbersOfArguments === 1) {
    return () => argumentsOfFunction[0];
  }
  return null;
}
...
function memoize(func) {
  ...
  return function returnFunctionResult() {
...
function logger(func, logFunc) {
  const nameOfFunction = func.name;
  return function returnLogginWrapper(...args) {
    const stringWithArguments = JSON.stringify(args);
    const pureString = stringWithArguments.substring(
      1,
      stringWithArguments.length - 1,
    );
    logFunc(`${nameOfFunction}(${pureString}) starts`);
    const resultOfFirstFuncton = func(...args);
    logFunc(`${nameOfFunction}(${pureString}) ends`);
    return resultOfFirstFuncton;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return function returnWithAllArguments(...args2) {
